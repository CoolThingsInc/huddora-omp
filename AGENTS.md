...
- Plugin tools are **bridge-only**. Do not use host `MCPManager`/`callTool` for register, room, send, watch, or doctor transport.
...
- Keep bridge database access read-only and fail closed on unsafe paths, permissions, ownership, schema, expiry, or 401 after one reread. SSE may reinitialize/reconnect once after its first 401; a second SSE auth failure requires reauth. Close/unwatch and clear bridge SSE/session on disable, disconnect, and shutdown. `off` means plugin tools/delivery are unavailable until the bridge is re-enabled.
- Commands: `/huddora bridge status|off|on`. `off` persists the opt-out; `on` reconnects the bridge and resumes room watch.
