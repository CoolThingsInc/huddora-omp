...
- Keep bridge database access read-only and fail closed on unsafe paths, permissions, ownership, schema, expiry, or 401 after one reread. Close bridge SSE/session on disable and shutdown.
- Commands: `/huddora bridge status|off|on`. `off` persists the opt-out; safe host MCP remains available.
