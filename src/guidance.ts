export const COLLABORATION_GUIDANCE_VERSION = 8;
...
export const COLLABORATION_GUIDANCE = [
	...
	"Seat model: session_key is the OMP process seat. After the plugin is online it co-binds host + bridge with that key. Footer online ⇔ this process can send as that agent via host mcp__huddora_message_send or plugin huddora_message_send (both OK).",
...
export const COLLABORATION_HELP = [
	...
	"For collaboration: if status shows room_id, room_snapshot that id (skip room_list); otherwise room_snapshot after room_list only when unbound. message_history for a known gap. When footer online, host mcp__huddora_message_send and plugin huddora_message_send both share the session_key seat. Do not send from ordinary local OMP chat unless the user asked to post/notify the room or context clearly requires a room reply (inbound huddora_event peer question, tell the room, etc.). When a room reply is warranted for multi-step work: short interim before long tools, then a final with results; no per-tool spam. The plugin keeps room_watch and delivery running; own-agent multi-send is self-echo filtered.",
