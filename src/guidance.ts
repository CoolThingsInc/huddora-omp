export const COLLABORATION_GUIDANCE_VERSION = 9;
...
export const COLLABORATION_GUIDANCE = [
	...
	"Seat model: session_key is the OMP process seat. After the plugin is online it co-binds host + bridge with that key. Footer here ⇔ this process can send as that agent via host mcp__huddora_message_send or plugin huddora_message_send (both OK).",
	"Presence (user-facing): Here ⇔ can post from this surface; Away = not here; Needs reconnect → /huddora connect (or wait for plugin auto rebind). Revoked is terminal.",
	...
	"Progressive multi-part (only when a room reply is warranted): you MAY call huddora_message_send (or host message_send) multiple times mid-turn. Send a short interim before long tools/subtasks, then a final with results/links. Do not post every tool step — only when progress advances human understanding. Soft spacing: avoid burst spam (prefer a few seconds between chunks). Own agent sends are self-echo filtered; multi-send is safe.",
	"Agent identity lifecycle is fully automatic and plugin-owned: register, heartbeat/online, host+bridge rebind after reconnect. Never call agent_register or agent_heartbeat. Never invent session_key. If tools return agent_not_bound, use /huddora connect or wait for plugin auto host+bridge rebind — do not fix bind yourself.",
...
export const COLLABORATION_HELP = [
	...
	"For collaboration: if status shows room_id, room_snapshot that id (skip room_list); otherwise room_snapshot after room_list only when unbound. message_history for a known gap. When footer here, host mcp__huddora_message_send and plugin huddora_message_send both share the session_key seat. Presence: Here ⇔ can send; Away = not here; Needs reconnect → /huddora connect (or wait auto rebind). Do not send from ordinary local OMP chat unless the user asked to post/notify the room or context clearly requires a room reply (inbound huddora_event peer question, tell the room, etc.). When a room reply is warranted for multi-step work: short interim before long tools, then a final with results; no per-tool spam. The plugin keeps room_watch and delivery running; own-agent multi-send is self-echo filtered.",
	"Do not manage agent identity. agent_register, agent_heartbeat, and session_key are plugin infrastructure only (file seat + auto-rebind + presence + host co-bind). Never invent session_key; never register/heartbeat from the model. On agent_not_bound: /huddora connect or wait for plugin auto host+bridge rebind.",
