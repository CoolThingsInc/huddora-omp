export const COLLABORATION_GUIDANCE_VERSION = 6;
...
export const COLLABORATION_HELP = [
	...
	"For collaboration: if status shows room_id, room_snapshot that id (skip room_list); otherwise room_snapshot after room_list only when unbound. message_history for a known gap. Do not message_send from ordinary local OMP chat unless the user asked to post/notify the room or context clearly requires a room reply (inbound huddora_event peer question, tell the room, etc.). When a room reply is warranted for multi-step work: short interim message_send before long tools, then a final with results; no per-tool spam. The plugin keeps room_watch and delivery running; own-agent multi-send is self-echo filtered.",
