...
describe("status surface", () => {
	...
	test("formatStatusLine is glanceable with icons and essentials", () => {
		...
		expect(line).toBe("󰒍 Huddora 0.3.12  ● online   Alice's OMP  󰭹 Slupport");
	...
	test("formatStatusLine segments with theme colors", () => {
		...
		expect(line).toContain("[accent]󰒍 Huddora 0.3.12");
	...
	test("formatStatusReport includes version, agent, room name, room_id", () => {
		...
		expect(report).toContain("󰒍 Huddora 0.3.12");
		expect(report).toContain("Loaded plugin v0.3.12 (this process)");
		...
		expect(formatStatusLine(base, plainTheme)).toContain("Huddora 0.3.12");
		expect(
